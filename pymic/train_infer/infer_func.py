...
def volume_infer(image, net, device, class_num, 
    ...
    if(mini_patch_inshape is None):
        ...
        outputs = volume_infer_by_patch(image, net, device, class_num,
            mini_batch_size, mini_patch_inshape, mini_patch_outshape, stride)
    return outputs
...
def volume_infer_by_patch(image, net, device, class_num,
    ...
    # nd = int(math.ceil(D/mini_patch_outshape[0]))
    # nh = int(math.ceil(H/mini_patch_outshape[1]))
    # nw = int(math.ceil(W/mini_patch_outshape[2]))
    sub_image_patches = []
    sub_image_starts  = []
    ...
    for mini_batch_idx in range(max_mini_batch):
        ...
        if(isinstance(out_mini_batch, tuple) or isinstance(out_mini_batch, list)):
            out_mini_batch = out_mini_batch[0] 
        out_mini_batch = out_mini_batch.cpu().numpy()
        mask_mini_batch = np.ones_like(out_mini_batch)
...
def volume_infer_by_patch_backup(image, net, device, class_num,
    ...
    for mini_batch_idx in range(max_mini_batch):
        ...
        if(isinstance(out_mini_batch, tuple) or isinstance(out_mini_batch, list)):
            out_mini_batch = out_mini_batch[0] 
        out_mini_batch = out_mini_batch.detatch().cpu().numpy()
        for batch_idx in range(batch_start_idx, batch_end_idx):
            crop_start = sub_image_starts[batch_idx]
            crop_start = [crop_start[i] + margin_lower[i] for i in range(img_dim)]
            crop_end   = [crop_start[i] + mini_patch_outshape[i] for i in range(img_dim)]
